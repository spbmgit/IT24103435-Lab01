...
class VisualGridHuntGame:
    ...
    def __init__(
        ...
        # Step 2.1: toxic traps, placed avoiding (0,0), walls, and food
    ...
    def get_percept(self) -> dict:
        """
        Fully observable: the agent's percept exposes the true global
        state of the environment (its own position, opponent positions,
        and whether it is currently standing on food/a trap/a wall).
        """
        return {
    ...
    def execute_action(self, action: str):
        ...
        # Step 2.3: penalty for stepping on a toxic trap
...
class GridGameGUI:
    ...
    def draw_grid(self):
        ...
        # Step 2.3: render toxic traps as purple circles
        for tx, ty in self.env.toxic_traps:
            x1 = tx * self.cell_size + 5
            y1 = (self.env.height - 1 - ty) * self.cell_size + 5
            x2 = (tx + 1) * self.cell_size - 5
            y2 = (self.env.height - ty) * self.cell_size - 5
            self.canvas.create_oval(x1, y1, x2, y2, fill="purple", outline="darkmagenta")
    ...
    def run_loop(self):
        ...
        def step():
            if not self.env.is_done():
                # Practical 01 baseline: no agent logic yet, just random movement
